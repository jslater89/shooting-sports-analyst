...
- **Place bands:** Place k occupies the continuous band [k−0.5, k+0.5]. For a range [L, R], full contribution (1.0) when s ∈ [L−0.5, R+0.5]. When L ≥ 2, a **smoothstep** ramp (t²(3−2t) over the unit interval) over [L−1, L−0.5] at the lower edge and over [R+0.5, R+1] at the upper edge. Smoothstep is C¹ at 0 and 1, so P_shifted(δ) has no kinks at half-integer δ and the optimizer is not drawn to settle at 0.5, 1.5, etc.
...
**Latent strength for top finishes (odds pricing in [probability.dart](lib/data/ranking/prediction/odds/probability.dart)).** Integer Monte Carlo ranks put every simulated **1st** on the same `place == 1`, and normalized ratio output often collapses wins to the same **1.0** value. A naive shift `s = place − δ` on integers then treats every win identically, which is too harsh for markets like “1st only” when δ is expressed in raw places. Without the full **N×N** competitor-performance matrix, the **pricing** path when `placeDelta` or `ratioDelta` is non-null uses the **marginal** MC sample and **empirical rank** to approximate **strength of victory** (blowout vs narrow win) in latent space:

- **Place (`PredictionProbability.fromPlacePrediction` with `placeDelta`):** Compute `meanPlace` and `stdDevPlace` on the full MC `places` list (same marginal as the info fields). Sort a **copy** of `places` ascending (best first). For each entry in that sorted list with index `i`, if `place ≤ 1`, form percentile `(i + 0.5) / N`, map it to a **z-score** with the normal quantile (`Normal.quantile`), then **latent place** `meanPlace + z × stdDevPlace`, capped with `min(1.0, …)`. Otherwise `latentPlace` is the integer place as a double. Then `s = placeShifted(latentPlace, placeDelta)` and `placeRangeContribution(s, …)` as usual. Percentile rank uses **order in the sorted list**, while μ and σ use the **unsorted** sample (all trials). This spreads mass among 1st-place trials: a 1st that sits low in the sorted order gets a **worse** latent coordinate than a 1st that sits high.

- **Percentage (`fromPercentagePrediction` with `ratioDelta`):** When `ratioDelta` is non-zero, sort `percentages` ascending and compute mean and std dev on that **sorted** list. For each entry with index `i`, when the raw ratio is **≥ 1.0** (normalized top / win), replace it with **percentile → z-score → mean + z × σ** on those sorted ratios (no `min(1, …)` on the implied value before adding `ratioDelta`). Then add `ratioDelta`. **Clamp** at **0** from below. Trials strictly **below 1.0** use `originalRatio + ratioDelta` with the same floor.

δ is still interpreted in **raw places** or **raw ratio points**; this step only **opens** the top bucket so that small negative shifts do not instantly erase every “win” trial when the model already had many tied-at-1st outcomes.

The δ **optimizer** (`calculateBayesianOddsUpdate`) evaluates `P_shifted` via [BayesianOddsWager.evaluatePlaceAgainstSimulation](lib/data/prediction_game/bayesian_odds/wager_data.dart) / `evaluatePercentAgainstSimulation`. Those methods may or may not mirror the same latent construction as `probability.dart`; keep them aligned if posteriors and displayed odds should be consistent.

### Generating Odds Under the Shifted Distribution

Once δ is computed, generating odds for a requested market uses [PredictionProbability.fromPlacePrediction](lib/data/ranking/prediction/odds/probability.dart) / `fromPercentagePrediction` / spread factories with the appropriate delta, including the **latent top-finish** handling above when δ is applied. Then apply the logit clamp (see [wager_updater.dart](lib/data/prediction_game/bayesian_odds/wager_updater.dart)). See the wager/odds pipeline for how δ is passed and probabilities are produced.
...
- **[place_evaluation.dart](lib/data/ranking/prediction/odds/place_evaluation.dart)** — `placeShifted(place, delta)`, `placeRangeContribution(s, bestPlace, worstPlace)` for continuous place evaluation (smoothstep ramps at boundaries so P_shifted is C¹ in δ).
...
- **[probability.dart](lib/data/ranking/prediction/odds/probability.dart)** — `PredictionProbability.fromRawProbability` builds a probability from a raw P for use in the logit clamp; `rawMoneylineOdds` for logging. Final decimal odds are clamped between `worstPossibleOdds` and `bestPossibleOdds` in `decimalOdds` getter. When `placeDelta` / `ratioDelta` is applied, `fromPlacePrediction` and `fromPercentagePrediction` use **latent top-finish** handling (sorted-sample percentile → normal quantile → mean + zσ) for 1st-place and ratio ≥ 1.0 trials; see “Latent strength for top finishes” under continuous place evaluation above.
...
Once δ is computed (or read from the BayesianDelta cache), generating odds for a requested market goes through `PredictionProbability` factories with the subject’s MC result and δ. For place and percentage, that includes **latent** construction for top buckets when δ is non-zero, as in [probability.dart](lib/data/ranking/prediction/odds/probability.dart) (see “Latent strength for top finishes” above), then the logit clamp (see "Maximum Odds Movement" and implementation in [wager_updater.dart](lib/data/prediction_game/bayesian_odds/wager_updater.dart)). Total weight on the shooter is summed over all wagers for evidence-dependent clamp width. No per-market state is stored; the same δ is used for every market on that shooter.
