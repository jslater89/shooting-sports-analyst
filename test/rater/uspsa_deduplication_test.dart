...
void main() async {
  ...
  test("Michael Morgan Ambiguous Mapping", () async {
    ...
    expect(reason: "number of results", results, hasLength(1));
    var result = results.first;
    expect(reason: "cause is ManualReviewRecommended", result.causes.firstWhereOrNull((e) => e is ManualReviewRecommended), isA<ManualReviewRecommended>());
    expect(reason: "cause is ImplicitBlacklist", result.causes.firstWhereOrNull((e) => e is ImplicitBlacklist), isA<ImplicitBlacklist>());
    var implicitBlacklist = result.causes.firstWhereOrNull((e) => e is ImplicitBlacklist) as ImplicitBlacklist;
    expect(reason: "implicit blacklist identity count", implicitBlacklist.identityCount, equals(2));
    var longIdentity = implicitBlacklist.identities.firstWhere((e) => e.length > 1);
    expect(reason: "long identity", longIdentity, unorderedEquals(["FY94000", "B96"]));
    var shortIdentity = implicitBlacklist.identities.firstWhere((e) => e.length == 1);
    expect(reason: "short identity", shortIdentity, equals(["A163595"]));
    ...
    expect(reason: "proposed blacklist source number", proposedBlacklist.sourceNumber, equals("B96"));
    expect(reason: "proposed blacklist target number", proposedBlacklist.targetNumber, equals("A163595"));
  });
  ...
  test("Michael Morgan Ambiguous Mapping 2", () async {
    ...
    expect(reason: "number of results", results, hasLength(1));
    var result = results.first;
    expect(reason: "number of causes", result.causes, hasLength(2));
    expect(reason: "cause is ManualReviewRecommended", result.causes.firstWhereOrNull((e) => e is ManualReviewRecommended), isA<ManualReviewRecommended>());
    expect(reason: "cause is ImplicitBlacklist", result.causes.firstWhereOrNull((e) => e is ImplicitBlacklist), isA<ImplicitBlacklist>());
    var implicitBlacklist = result.causes.firstWhereOrNull((e) => e is ImplicitBlacklist) as ImplicitBlacklist;
    expect(reason: "implicit blacklist identity count", implicitBlacklist.identityCount, equals(2));
...
Future<List<DbShooterRating>> addMatchToTest(AnalystDatabase db, DbRatingProject project, String matchId) async {
  ...
  for(var competitor in match.shooters) {
    var r = DbShooterRating(
      ...
      rating: 1000,
      agedRating: 1000,
      careerMinimumRating: 1000,
      careerMaximumRating: 1000,
