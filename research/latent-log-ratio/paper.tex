...
\begin{document}
	...
	Because competitors cannot draw infinitely from an upper Gaussian tail, the actual maximum order statistic of an elite field is highly compressed. To model this physical ``speed wall,'' we introduce a structural asymmetry parameter $\theta_{\mathrm{up}} \in [0.35, 0.60]$ to dampen the expected extreme value shift. 
	
	To preserve Extreme Value Theory assumptions regarding cohort covariance, we must also strictly isolate the independent fraction of the variance. We partition the epistemic uncertainty using $(1 - \rho)$, and rely on the purely idiosyncratic prediction sport noise $\sigma_{\mathrm{pred}}^2$ to represent match-day variance. The presumed winner's rating is shifted upward by this corrected, dampened extreme-value expectation:
	\begin{equation}
		\Delta_w = \theta_{\mathrm{up}} \cdot \Phi^{-1}\Big(\frac{N_\mathrm{eff} - 0.375}{N_\mathrm{eff} + 0.25}\Big) \cdot \sqrt{V_w^{\mathrm{age}}(1 - \rho) + (\kappa \cdot \tilde{\sigma}_w^2) + \sigma_{\mathrm{pred}}^2}
	\end{equation}
