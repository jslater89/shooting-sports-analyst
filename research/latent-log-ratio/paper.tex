...
\begin{document}
	...
	By taking the natural logarithm of match percentages, we completely eliminate the winner's influence when comparing any two competitors. Because $\ln(a/b) = \ln(a) - \ln(b)$, the log-difference between two competitors becomes a pure measurement of their underlying performance ratio, regardless of the scoring format:
	
	For Hit Factor:
	\begin{equation}
		\ln(S_A) - \ln(S_B) = \ln\left(\frac{P_A}{1.0}\right) - \ln\left(\frac{P_B}{1.0}\right) = \ln\left(\frac{P_A}{P_B}\right)
	\end{equation}
	
	For Time-Plus:
	\begin{equation}
		\ln(S_A) - \ln(S_B) = \ln\left(\frac{T_{min}}{T_A}\right) - \ln\left(\frac{T_{min}}{T_B}\right) = \ln\left(\frac{T_B}{T_A}\right)
	\end{equation}
	
	In both cases, the anchor (the winner) vanishes.
	...
	The rating difference \textit{is} the expected performance difference: a rating gap of $\ln(2) \approx 0.69$ means one competitor is expected to perform at twice the level of the other. This relationship is exact in log space and carries through the entire update and prediction pipeline without distortion. No probability function, score function, or scaling parameter mediates between rating and performance---they are the same quantity in different representations.
	...
	Floor match percentages at a minimal value (e.g., 0.01) to prevent undefined logarithms, then convert to log space:
	\begin{equation}
		L_i = \ln(S_i)
	\end{equation}
	
	\textbf{Step 2: Calculate Competitor Weights}\\
	Determine how much influence a competitor should have on establishing the match's difficulty. High-variance and high-volatility competitors have less influence:
	\begin{equation}
		W_i = \frac{1}{V_i + \sigma_{sport}^2 + \sigma_i^2}
	\end{equation}
	
	\textbf{Step 3: Establish the Match Baseline ($B$)}\\
	Calculate the latent difficulty of the match by comparing the field's expected ratings against their actual log-scores:
	\begin{equation}
		B = \frac{\sum_{i=1}^N W_i (R_i - L_i)}{\sum_{i=1}^N W_i}
	\end{equation}
	...
	\end{equation}
	This measures how much better or worse competitor $i$ performed against opponent $j$ relative to what their ratings predicted. The winner's curse is already eliminated by the log transformation.
	
	Compute a precision-weighted average of the pairwise residuals:
	\begin{equation}
		D_i = \frac{\sum_{j \in \mathcal{S}_i} w_j \cdot \delta_{ij}}{\sum_{j \in \mathcal{S}_i} w_j}, \quad w_j = \frac{1}{V_j + \sigma_{sport}^2 + \sigma_j^2}
	\end{equation}
	...
	To avoid circularity (competitor $i$ influencing their own baseline), replace $B$ with a leave-one-out estimate:
	\begin{equation}
		B_{-i} = \frac{\sum_{j \neq i} W_j (R_j - L_j)}{\sum_{j \neq i} W_j}
	\end{equation}
	
	The variance of this estimate under the precision-weighted mean is:
	\begin{equation}
		\mathrm{Var}(B_{-i}) = \frac{1}{\sum_{j \neq i} W_j}
	\end{equation}
	
	In large fields with well-known competitors, $\sum_{j \neq i} W_j$ is large and $\mathrm{Var}(B_{-i})$ is negligible. In small fields, particularly those dominated by high-variance newcomers, $\mathrm{Var}(B_{-i})$ grows substantially---exactly when the model should be cautious.
	...
	\begin{equation}
		R_{obs,i} = \sigma_{sport}^2 + \sigma_i^2 + \mathrm{Var}(B_{-i}) + \alpha^2 \cdot \mathrm{Var}(D_i)
	\end{equation}
	...
	The effect is automatic and principled: large fields with strong opponents produce small $R_{obs,i}$ (close to the original $\sigma_{sport}^2 + \sigma_i^2$), so rating updates proceed normally. Small fields with uncertain opponents inflate $R_{obs,i}$, which simultaneously reduces $K_i$ (smaller updates) and reduces $|z_i|$ (making the Student-$t$ robust weight less aggressive). No hand-tuned field-size multipliers or evidence scale-down factors are needed.
