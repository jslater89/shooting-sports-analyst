...
\begin{document}
	...
	\begin{enumerate}
		...
		\item \textbf{Robust Mean Update:} The latent rating \(R_i\) is updated using a Kalman filter with gain \(K_i = V_i^{\mathrm{prior}} / (V_i^{\mathrm{prior}} + R_{\mathrm{obs},i})\), where \(V_i^{\mathrm{prior}}\) is the Trend-inflated prior defined in the next step. A Student-$t$ robust weighting \(w_t(z_i)\) is applied exclusively to the mean update to limit the influence of extreme single-event anomalies.
	...
	This placement carries distinct statistical meaning. Trend (injected as process noise on the prior) represents filter-validated evidence of directional drift: it is \emph{already} the output of an EMA that smooths away zero-mean scatter, so it is safe to let it raise $K_i$ and therefore move the mean. Shock (injected as a posterior variance bump) is a provisional response to a single-event surprise: it widens the epistemic uncertainty so that subsequent matches see a larger prior (via Step 10's aging step and Step 7's inflated prior on the next iteration), but it does not force the current mean to chase an unvalidated outlier. If the shock presages a true skill shift, the next match's higher prior will yield a larger $K_i$ and a correspondingly larger mean response; if it was a one-off, the posterior contraction on subsequent normal events returns $V_i$ to its pre-shock level within a few matches without having whipped the mean around.
