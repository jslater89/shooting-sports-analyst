...
\begin{document}
	...
	\begin{equation}
		p_w \approx \prod_{j \neq w} \Phi\!\left(\frac{R_w - R_j}{\sqrt{V_w + V_j + 2\sigma_{sport}^2 + \sigma_w^2 + \sigma_j^2}}\right)
	\end{equation}
	...
	\begin{enumerate}
		\item For each competitor $i$, draw a performance sample $X_i \sim \mathcal{N}(R_i, V_i + \sigma_{sport}^2 + \sigma_i^2)$.
	...
	In practice, filter to $p_w > \epsilon$ (e.g., $\epsilon = 0.005$) before computing the prediction mixture to avoid noise from single-trial long-shot winners. If filtering, renormalize such that $\sum_w p_w = 1$.
	...
	For a single presumed winner $w$ with performance $X_w \sim \mathcal{N}(R_w, V_w + \sigma_{sport}^2 + \sigma_w^2)$:
	\begin{equation}
		E[\exp(R_i - X_w)] = \exp\!\left(R_i - R_w + \frac{V_w + \sigma_{sport}^2 + \sigma_w^2}{2}\right)
	\end{equation}
	
	The correction term $\frac{V_w + \sigma_{sport}^2 + \sigma_w^2}{2}$ is always positive: when the winner's rating is uncertain and the winner's performance is also subject to volatility, the expected finish percentage is higher than the naive point estimate, because the winner is more likely to have a mediocre day than a perfect one.
	
	Combining with the winner mixture:
	\begin{equation}
		\hat{S}_i = \sum_{w \in \text{top-}m} p_w \cdot \exp\!\left(R_i - R_w + \frac{V_w + \sigma_{sport}^2 + \sigma_w^2}{2}\right)
	\end{equation}
	...
	\begin{enumerate}
		...
		\item The presumed winner's uncertainty (probability-weighted $V_w$)
		\item The presumed winner's behavioral volatility (probability-weighted $\sigma_w^2$)
	...
	\begin{equation}
		\sigma_{pred,i}^2 = V_i + \sigma_i^2 + \sigma_{sport}^2 + \sum_{w \in \text{top-}m} p_w \left( V_w + \sigma_w^2 \right)
	\end{equation}
